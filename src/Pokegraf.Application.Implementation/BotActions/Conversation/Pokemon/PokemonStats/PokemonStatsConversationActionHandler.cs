...
namespace Pokegraf.Application.Implementation.BotActions.Conversation.Pokemon.PokemonStats
{
    public class PokemonStatsConversationActionHandler : Pokegraf.Common.Request.RequestHandler<PokemonStatsConversationAction, Result>
    {
        ...
        public PokemonStatsConversationActionHandler(ILogger<Pokegraf.Common.Request.RequestHandler<PokemonStatsConversationAction, Result>> logger, IMediator mediatR, IPokemonService pokemonService) : base(logger, mediatR)
        {
            _pokemonService = pokemonService;
        }

        public override async Task<Result> Handle(PokemonStatsConversationAction request, CancellationToken cancellationToken)
        {
            ...
            var keyboard = pokemonResult.Value.ToStatsKeyboard();

            return await MediatR.Send(new PhotoWithCaptionWithKeyboardResponse(pokemonResult.Value.Image.ToString(), pokemonResult.Value.ToStatsCaption(), keyboard));
        }
    }
}